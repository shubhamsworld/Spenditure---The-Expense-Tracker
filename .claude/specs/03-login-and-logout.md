...
## Overview
This feature implements user authentication for spenditure. It converts the `/login` stub into a functional POST handler that verifies credentials against the database, stores the authenticated user's ID in the session, and redirects to the dashboard (or a suitable landing page). It also implements the `/logout` stub, which clears the session and redirects to the landing page. After this step, the app can distinguish logged-in users from guests, which is a prerequisite for all expense features.
...
- [ ] Submitting the form with valid credentials (e.g. demo@spenditure.com / demo123) sets `session["user_id"]` and redirects to `/`
